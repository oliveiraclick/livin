...
export const CategoryFeed: React.FC = () => {
  ...
  // Filtra prestadores pela categoria
  const providers = MOCK_PROVIDERS.filter(p => p.category === categoryName);

  // Extrai subcategorias únicas
  const subcategories = ['Todos', ...Array.from(new Set(providers.map(p => p.subcategory)))];

  // Filtro final
  const filteredList = activeSub === 'Todos' 
    ? providers 
    : providers.filter(p => p.subcategory === activeSub);
           ...
           filteredList.map(provider => (
             ...
             >
                <div className="w-24 h-24 rounded-[24px] overflow-hidden relative shrink-0">
                   ...
                   {provider.isResident && (
                     <div className="absolute bottom-0 left-0 right-0 bg-violet-600/90 text-white text-[8px] font-bold text-center py-1 uppercase tracking-widest">
                        Vizinho
                     </div>
                   )}
                </div>
