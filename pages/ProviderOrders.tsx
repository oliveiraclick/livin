...
export const ProviderOrders: React.FC = () => {
  ...
  // Using Mock Data from the first provider
  const [orders, setOrders] = useState<Order[]>(MOCK_PROVIDERS[0].orders || []);
           ...
           filterOrders(activeTab).map(order => (
             <div key={order.id} className="bg-white rounded-[24px] p-5 shadow-soft border border-slate-50">
                ...
                <div className="flex justify-between items-center mb-6">
                   ...
                   <span className="text-xl font-black text-violet-600">R$ {order.total.toFixed(2)}</span>
                </div>
