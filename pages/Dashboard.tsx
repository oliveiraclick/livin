    ...
    <div className="min-h-screen bg-[#f8fafc] pb-40">
      ...
      <div className="pl-6 overflow-x-auto whitespace-nowrap scrollbar-hide pb-8 flex gap-6 snap-x snap-mandatory">
         {filteredProviders.map((provider) => (
            ...
            >
               <img src={provider.coverImage} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
               <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/20 to-transparent"></div>
               
               {/* Floating Tag */}
               <div className="absolute top-4 left-4">
                  <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider backdrop-blur-md border border-white/20 ${provider.isResident ? 'bg-white/20 text-white' : 'bg-black/40 text-white'}`}>
                    {provider.isResident ? 'Morador' : 'Profissional'}
                  </span>
               </div>

               <div className="absolute bottom-0 left-0 right-0 p-4">
                  <div className="flex justify-between items-end mb-1">
                     <span className="text-accent-500 font-bold text-[9px] uppercase tracking-wider truncate max-w-[80px]">{provider.subcategory}</span>
                  ...
                  <h3 className="text-lg font-black text-white leading-tight whitespace-normal line-clamp-2">{provider.name}</h3>
                  {provider.isPresent && (
