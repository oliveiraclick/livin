...
export const ProviderProfile: React.FC = () => {
  ...
  const products = provider?.offers?.filter(o => o.type === 'product') || [];
  const services = provider?.offers?.filter(o => o.type === 'service') || [];
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'services'>(
      mappedInitialTab || (hasServices ? 'services' : 'products')
  );

  // Show hybrid choice modal if provider has both and no tab was pre-selected
  useEffect(() => {
     if (hasProducts && hasServices && !passedTab) {
        setShowHybridModal(true);
     }
  }, [hasProducts, hasServices, passedTab]);

  if (!provider) return <div>404</div>;
    ...
    return (
      <div className="fixed inset-0 z-[200] bg-gradient-to-br from-violet-600 to-fuchsia-600 flex flex-col items-center justify-center p-8 text-center animate-fade-in text-white">
         ...
         <p className="text-white/80 font-medium max-w-xs mx-auto mb-8">
            O prestador <strong>{provider.name}</strong> recebeu seu pedido. Você pode acompanhar o status na aba "Pedidos".
         </p>
         <button 
  ...
  return (
    <div className="min-h-screen bg-white pb-32">
      ...
      <div className="-mt-10 relative bg-white rounded-t-[40px] px-6 pt-10 min-h-screen">
         <div className="flex justify-between items-start mb-2">
            <h1 className="text-3xl font-black text-gray-900 leading-tight w-3/4">{provider.name}</h1>
         ...
         <p className="text-gray-500 font-medium text-sm mb-6">{provider.category} • {provider.subcategory}</p>

         <div className="flex gap-3 mb-8">
            <span className={`px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wide ${provider.isResident ? 'bg-violet-50 text-violet-700' : 'bg-gray-100 text-gray-600'}`}>
               {provider.isResident ? 'Vizinho' : 'Profissional'}
            </span>
            {provider.isPresent && (
      ...
      {itemCount === 0 && (
         <div className="fixed bottom-6 left-6 right-6 z-[90]">
           <Button fullWidth className="shadow-2xl shadow-violet-500/40" onClick={() => window.open(`https://wa.me/${provider.phone || ''}`, '_blank')}>
              <MessageCircle size={20} /> Falar com {provider.name.split(' ')[0]}
           </Button>
         </div>
      )}
      ...
      {showHybridModal && (
        <div className="fixed inset-0 z-[110] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in">
           <div className="bg-white w-full max-w-sm rounded-[32px] p-8 text-center animate-slide-up shadow-2xl relative">
              ...
              <p className="text-slate-500 font-bold mb-8">Como a {provider.name} pode te ajudar hoje?</p>
